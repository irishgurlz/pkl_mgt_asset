<form action="/distribution-asset" id="form-add-device" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="row">
        <div class="col col-3">
            <div class="mb-2">
                ...
                <input type="text" class="form-control" name="nomor_penyerahan" id="nomor_penyerahan_hidden" value="{{ old('nomor_penyerahan', session('nomor_penyerahan')) }}" style="background-color: #f2f2f2;" readonly>
        ...
        <div class="col col-3 d-flex align-items-center">
            <div class="mt-3" style="width:100%">
                <button class="btn btn-success" type="button" style="width:100%" id="add-device-btn">Tambah Perangkat Komputer</button>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        ...
        @endif
        <div class="col-12 d-flex flex-wrap" id="form-device-container">
                ...
                <div class="col-6 me-2 form-asset" id="form-asset-{{ $key + 1 }}" style="width: 49%">
                    <div class="card shadow mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                <label for="nomor_it_{{ $key }}">Nomor Asset</label>
                                <input type="text" name="nomor_it[]" id="nomor_it_{{ $key }}" value="{{ old('nomor_it.' . $key) }}" placeholder="Masukkan Nomor Asset" class="form-control">
                                <div class="nomor_it-status text-danger"></div>
                                @error("nomor_it.{$key}")
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById("add-device-btn").addEventListener("click", function () {
            ...
            const newFormDevice = `
            <div class="col-6 me-2 form-asset" id="form-device-${assetCount}" style="width: 49%">
                <div class="card shadow mb-3">
                    <div class="card-header py-3 text-center">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label for="nomor_it_${assetCount}">Nomor Asset</label>
                            <input type="text" name="nomor_it[]" id="nomor_it_${assetCount}" placeholder="Masukkan Nomor Asset" class="form-control" required>
                            <div class="nomor_it-status text-danger"></div>
                        </div>
                        <div class="form-group">
            ...
            const formContainer = document.getElementById("form-device-container");
            formContainer.insertAdjacentHTML("beforeend", newFormDevice);
...
$(document).on('input', 'input[name="nomor_it[]"]', function() {
    var nomor_it = $(this).val().trim();
    var nomorItStatus = $(this).closest('.form-asset').find('.nomor_it-status'); 

    var existingNomorIt = [];
    $('input[name="nomor_it[]"]').each(function() {
        if ($(this).val().trim() !== '') {
            existingNomorIt.push($(this).val().trim());
        }
    });

    var duplicateCount = existingNomorIt.filter(it => it === nomor_it).length;

    if (duplicateCount > 1) {
        nomorItStatus.text('Nomor Asset sudah digunakan dalam form ini').removeClass('text-success').addClass('text-danger');
        return;
    }

    if (nomor_it) {
        $.ajax({
            url: '{{ route('checkNomorITDist') }}',
            type: 'POST',
            data: {
                nomor_it: nomor_it,
                _token: '{{ csrf_token() }}'
            ...
            success: function(response) {
                if (response.status === 'not_registered') {
                    nomorItStatus.text('Nomor Asset belum terdaftar').removeClass('text-success').addClass('text-danger');
                } else if (response.status === 'available') {
                    nomorItStatus.text(response.message + ' (Kondisi: ' + response.kondisi + ')')
                    .removeClass('text-danger').addClass('text-success');
            ...
            error: function(xhr) {
                // nomorItStatus.text('Terjadi kesalahan, coba lagi nanti').remov   eClass('text-success').addClass('text-danger');
            }
        });
    } else {
        nomorItStatus.text('');
